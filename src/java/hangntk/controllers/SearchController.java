...
public class SearchController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<TblArticle> result = blo.searchLikeContent(search, currentPage, MAX_RECORD);
            long total = blo.totalArticleSearch(search);
            total = total / MAX_RECORD;
            total += 1;
            request.setAttribute("CURRENT", currentPage);
            request.setAttribute("TOTAL", total);
            request.setAttribute("SEARCH", search);
            request.setAttribute("LISTARTICLE", result);
        } catch (Exception e) {
